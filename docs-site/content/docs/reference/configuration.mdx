...
| `OPEN_CURSOR_SDK_SYSTEM_PROMPT_MODE` | `message` | Selects `message` to include OpenCode system messages in the normal SDK prompt, or `replace` to use them as Cursor's replacement system prompt. `replace` requires Cursor to enable that capability for the account. | Your Cursor account supports replacement system prompts and you need OpenCode's prompt to replace Cursor's built-in prompt. |
